  ...
  <div class="home">
    ...
    <v-dialog v-model="newProjectDialog" max-width="95%">
      <v-card>
        ...
        <v-card-subtitle>You will automatically be set as owner</v-card-subtitle>
        <v-text-field
    ...
    <v-dialog v-model="newTreeDialog" max-width="95%">
      <v-card>
        ...
        />
        <v-text-field
          v-model="newTreeDescription"
        />
        <v-card-actions>
          <v-btn
            @click="newTreeDialog = false"
            >
            cancel
          </v-btn>
          <v-btn
      ...
      <v-expansion-panel v-for="project in projectList" :key="project.id">
        ...
        <v-expansion-panel-content>
          ...
          <v-row>
            ...
            <v-col class="shrink">
              <v-btn disabled v-on:click="deleteProject(project.id)">delete</v-btn>
            </v-col>
          </v-row>
      ...
      <!-- subproject table -->
              <tbody v-if="project.subprojects.length">
                <tr v-for="sp in project.subprojects.filter(sp => sp.tree)" :key="sp.id">
                  <td class="text-left">
                    <router-link
                      :to="{ name: 'efm-project', params: { treeID: sp.tree.id } }"
                    >
                      {{ sp.tree.name }}
                    </router-link>
                  </td>
                  ...
                  <td>
                    ...
                    >
                      ...
                      <span>Delete subproject and EFM-tree "{{sp.tree.name}}"</span>
                    </v-tooltip>
                  </td>
                </tr>
              </tbody>
...
export default {
  ...
  methods: {
    ...
    async saveNewProject() {
      await this.$store.dispatch('newProject', {projectName: this.newProjectName})
    ...
    async saveNewTree(forward= false) {
      ...
      let theNewTree = await this.$store.dispatch('efm/newTree', {projectID: this.newTreeProject, treeData: treeData})
      if (theNewTree) {
        this.newTreeName = ""
        ...
        if (forward) {
          ...
          this.$router.push({ name: 'efm-project', params: { treeID: theNewTree.id} })
        }
      }
    },
    ...
    async deleteSubproject() {
      const deleted = await this.$store.dispatch('efm/deleteTree', {treeID: this.toDeleteID})
      if (deleted) {
        this.toDeleteID = null
      }
    }
