...
export default {
    ...
    methods: {
        async save() {
            ...
            // add required fields:
            for (const [api, field] of Object.entries(this.objectInfo.requiredFields)) {
            ...
            for (const [api, field] of  Object.entries(this.objectInfo.optionalFields)) {
                if (this.newObject[field]) {
                    // set to API value
                    objectToSubmit[api] = this.newObject[field]
                }
        ...
        loadObject() {
            if (this.editID) {
                ...
                let parentIDinAPI = Object.keys(this.objectInfo.requiredFields).find(key => this.objectInfo.requiredFields[key] === 'parentID');
                if (parentIDinAPI) {
