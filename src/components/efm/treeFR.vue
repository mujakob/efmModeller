<template>
  <div class="float-left">
    <v-card color="blue" width="300px">
      <v-card-title>{{ theFR.name }}</v-card-title>
      <v-card-text>{{ theFR.description }}</v-card-text>
      <v-card-actions>
        <v-btn @click="newChild"> new function </v-btn>
        <v-btn @click="editSelf"> edit </v-btn>
        <v-btn @click="deleteSelf"> delete </v-btn>
        <v-btn @click="showChildren != showChildren" fab small>
    ...
    <treeDS v-for="dsID in theFR.is_solved_by_id" :key="dsID" :dsID="dsID" />
  </div>
</template>
